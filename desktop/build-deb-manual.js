...
`;
  ...
  // Copy icon
  const iconDir = join(debianDir, 'usr', 'share', 'icons', 'hicolor', '1024x1024', 'apps');
  mkdirSync(iconDir, { recursive: true });
  execSync(`cp desktop/assets/icon.png "${iconDir}/turfslot-desktop.png"`, { stdio: 'pipe' });
  
  // Copy .desktop file
  const appsDir = join(debianDir, 'usr', 'share', 'applications');
  mkdirSync(appsDir, { recursive: true });
  execSync(`cp desktop/assets/turfslot-desktop.desktop "${appsDir}/"`, { stdio: 'pipe' });

  console.log('🔨 Building .deb package...');
  execSync(`dpkg-deb --build --root-owner-group "${debianDir}" "${debFile}"`, { stdio: 'pipe' });

  console.log(`✅ Successfully built: ${debFile}`);
  
  const sizeOutput = execSync(`ls -lh "${debFile}" | awk '{print $5}'`, { encoding: 'utf8' }).trim();
