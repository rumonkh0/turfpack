...
export const cashPosition = asyncHandler(async (req, res) => {
  const data = await getCashPosition(req.query);
  res.status(200).json({ success: true, data });
});

export const receivables = asyncHandler(async (req, res) => {
  const data = await getReceivables(req.query);
  res.status(200).json({ success: true, data });
});

export const partnerShares = asyncHandler(async (req, res) => {
  const data = await getPartnerShares(req.query, req.user);
  res.status(200).json({ success: true, data });
});

export const revenueBreakdown = asyncHandler(async (req, res) => {
  const pnl = await getProfitLoss(req.query);
...
export const dashboard = asyncHandler(async (req, res) => {
  const data = await getDashboard(req.query, req.user);
  res.status(200).json({ success: true, data });
});
