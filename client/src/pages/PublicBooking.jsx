...
export default function PublicBooking() {
  ...
  const hours = selectedTurf
    ? Array.from(
        {
          length:
            (selectedTurf.closing_hour || 23) -
            (selectedTurf.opening_hour || 6),
        },
        (_, i) => i + (selectedTurf.opening_hour || 6),
      )
    : [];
  ...
  const price = useMemo(
    () =>
      startHour !== null && endHour !== null
        ? calcPrice(selectedTurf, selectedDate, startHour, endHour)
        : 0,
        ...
        <div className="flex items-center justify-center gap-2 mb-8">
          {[1, 2, 3].map((s) => (
            <div key={s} className="flex items-center gap-2">
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold transition-colors ${step >= s ? "bg-emerald-600 text-white" : "bg-gray-200 text-gray-400"}`}
              >
                {s}
              </div>
              <span
                className={`text-xs hidden sm:inline ${step >= s ? "text-emerald-700" : "text-gray-400"}`}
              >
                {s === 1
                  ? "Select Turf"
                  : s === 2
              ...
              {s < 3 && (
                <div
                  className={`w-8 h-px ${step > s ? "bg-emerald-400" : "bg-gray-200"}`}
                />
              )}
            </div>
          ))}
                ...
                >
                  ...
                  <div className="p-3">
                    ...
                    <p className="text-xs text-gray-400">
                      {t.type?.replace(/-/g, " ")} · {t.size}
                    </p>
                    <div className="flex justify-between items-center mt-2">
                      <span className="text-xs text-gray-400 flex items-center gap-1">
                        <Clock className="w-3 h-3" />
                        {formatTime(t.opening_hour || 6)}–
                        {formatTime(t.closing_hour || 23)}
                      </span>
                ...
                <div className="flex justify-between items-center">
                  <div>
                    <p className="text-sm font-medium text-emerald-800">
                      {formatTime(startHour)} – {formatTime(endHour)} (
                      {(endHour - startHour).toFixed(2)}hr)
                    </p>
